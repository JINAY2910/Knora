...
interface TestHintChatbotProps {
    isOpen: boolean;
    onClose: () => void;
...
export default function TestHintChatbot({
    isOpen,
    onClose,
    questionNumber,
    questionText,
    topic,
    tags,
}: TestHintChatbotProps) {
    const systemContext = `You are the Knora AI Tutor helping a student during a Java programming exam. The student is currently on Question ${questionNumber} about the topic "${topic}" (tags: ${tags.join(", ")}). The question is: "${questionText}". Provide helpful hints and guidance WITHOUT giving away the direct answer. Encourage the student to think through the problem. Keep responses concise and focused.`;
    ...
    const [messages, setMessages] = useState<ChatMessage[]>([
        {
            ...
            parts: [
                {
                    text: `I'm here to help with Q${questionNumber} on **${topic}**! I'll guide you with hints without spoiling the answer. What's confusing you about this question?`,
                },
            ],
        },
    ]);
    ...
    const sendMessage = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            console.error("Hint chat error:", error);
            setMessages((prev) => [
                ...prev,
            ...
            <div className="bg-charcoal text-cream p-4 flex justify-between items-center border-b-2 border-charcoal">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-gray-400 tracking-widest uppercase truncate max-w-[220px]">
                            Q{questionNumber} · {topic}
                        </p>
                    </div>
                </div>
                <button onClick={onClose} className="text-cream hover:text-crimson transition-colors">
